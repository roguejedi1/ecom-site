...
const middleware = {
    ...
    isReviewAuthor: async(req,res,next) => {
        ...
        req.session.error = 'Not Authorised';
        return res.redirect('/');
    },
    ...
    isValidPassword: async(req, res, next) => {
        let { user } = await User.authenticate()(req.user.username, req.body.currentPassword);
        if(user){
            res.locals.user = user;
            next();
        } else {
            middleware.deleteProfileImage(req);
            req.session.error = 'Wrong password idiot';
